// 页面列表
<template>
  <div class="pageListCss" v-loading="loading">
...
import {
  ...
  deletePageApi,
  copyPageApi,
  getMrmsProjectInfoSelect,
  updatePageUpdateStatusApi,
} from "@/api/tool.js";
...
const statusClick = ({ row, column, value }) => {
  console.log(row, "row", row.pageStatus == "1");
  let obj = {
    pageId: row.pageId,
    pageStatus: row.pageStatus,
  };
  updatePageUpdateStatusApi(obj).then((res) => {
...
const bindData = reactive({
  ...
  apiUrl: "/business/mrmsTableManger/getMrmsProjectInfoByPage", //  接口地址
  showBorder: true, //  是否需要边框
  designTableColumns: [
    { columnLabel: "所属应用", columnName: "projectName", columnWidth: "160" },
    { columnLabel: "报表名称", columnName: "pageName", columnWidth: "300" },
    { columnLabel: "页面路径", columnName: "pageAddr" },
    {
      columnLabel: "启用否",
      kyf: "1",
      columnName: "pageStatus",
      align: "center",
      columnWidth: "100",
...
const btnClick = (item, row) => {
  try {
    loading.value = true;
    if (item.btnType == "add") {
      router.push({ path: "/setPageBusiness", query: {} });
    } else if (item.btnType == "edit") {
      router.push({
        path: "/setPageBusiness",
        query: {
          code: row?.pageAddr, //  页面路径
      ...
      ElMessageBox.confirm("是否确认复制选中的数据源？", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }).then(async () => {
