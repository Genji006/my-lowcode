    ...
    <el-card class="config-card">
        ...
        <div class="section-header" style="margin-left: 20px">
          ...
          >
            <el-row>
              <el-col :span="12"
                ><div class="grid-content bg-purple">
                  <el-form-item label="保存按钮文本00：" class="config-items">
                    <el-input
                      v-model="formData.saveText"
                      placeholder="如：保存"
                      size="small"
                    />
                  </el-form-item></div
              ...
              <el-col :span="12"
                ><div class="grid-content bg-purple-light">
                  <el-form-item label="取消按钮文本：" class="config-items">
                    <el-input
                      v-model="formData.cancleText"
                      placeholder="如：取消"
                      size="small"
                    />
                  </el-form-item></div
            ...
            <el-row>
              ...
              <el-col :span="12"
                ><div class="grid-content bg-purple-light">
                  <el-form-item label="是否开启行内表单：" class="config-items">
                    <el-switch
                      v-model="formData.isInline"
                      inactive-text="否"
                      active-text="是"
                      active-color="#13ce66"
                      inactive-color="#ff4949"
            ...
            <el-row>
              ...
              <el-col :span="12"
                ><div class="grid-content bg-purple-light">
                  ...
                  >
                    <el-radio-group
                      v-model="formData.titlePosition"
                      size="small"
                    >
            ...
            <el-row>
              <el-col :span="12">
                <div class="grid-content bg-purple">
                  <el-form-item label="标签位置：" class="config-items">
                    <el-radio-group
              ...
              <el-col :span="12">
                <div class="grid-content bg-purple">
                  <el-form-item label="标签宽度：" class="config-items">
                      <el-input
      ...
      <div v-if="formData.fields.length > 0" class="field-list">
        ...
        >
          ...
          />
          <el-table-column prop="prop" label="是否校验" align="center" width="130">
            ...
            <el-table-column prop="prop" label="正则校验" align="center" width="130">
             
            <template #default="scope">
             <el-select v-model="scope.row.regular" placeholder="请选择" clearable @change="handleRegularChange($event, scope.row)">
             <el-option v-for="item in fieldValidate()" :key="item.value" :label="item.label" :value="item.value" />
             </el-select>
            </template>
          </el-table-column>
          <el-table-column prop="prop" label="组件宽度" align="center">
            <template #default="scope">
              <el-input v-model="scope.row.width" placeholder="请输入组件宽度" />
            </template>
          </el-table-column>
...
// 处理正则校验字段变化，自动设置pattern字段
const handleRegularChange = (value, row) => {
  if (value) {
    // 查找fieldValidate中对应的项
    const validateItem = fieldValidate().find(item => item.value === value);
    if (validateItem) {
      // 设置pattern字段的值
      row.pattern =validateItem.pattern.toString();
    }
  } else {
    // 如果regular被清空，pattern也清空
    row.pattern = null;
  }
};
...
const rowEditValueFn = (type, item) => {
  ...
  if (type == "新增") {
    ...
    return {
      ...
      regular: item.regular, // 正则表达式
      pattern: item.pattern, // 正则表达式对象,
...
const rowDataFn = (type, item) => {
  let obj = {
    ...
    width: "220px", //  组件宽度（兼容直接设置）
    regular: currentField.regular || null, //  正则表达式验证
    pattern: currentField.pattern || null, //  正则表达式对象
...
const handleSubmit = () => {
  ...
  // let fieldCopy = JSON.parse(JSON.stringify(formData.fields));
  // fieldCopy.forEach((item) => {
  //   fieldValidate().forEach((value) => {
  //     if(item.regular == value.value){
  //       item.regular = value.pattern
  //     }
  //   })
  // });
  console.log(formData.fields, "formData.fields");
