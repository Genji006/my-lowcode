  ...
  <div class="table-config-panel">
    ...
    <div class="config-section">
      ...
      <el-form :model="formData" :inline="true" label-width="auto" :rules="formRules" class="config-form">
        ...
        <el-form-item label="默认高亮行">
          <el-input-number v-model="formData.defaultGetTable" :min="1" :max="999" />
        </el-form-item>
        <el-form-item label="是否默认选中" class="config-items">
          <el-switch v-model="formData.clickFirstRow" :active-value="true" :inactive-value="false" />
        </el-form-item>
    ...
    <div class="config-section">
      ...
      <el-form :model="formData" class="config-form">
        ...
        <el-form-item :label="`${formData.dsType === 'API' ? '接口路径：' : '查询语句：'}`" class="config-item full-width">
          <el-input v-model="formData.dsConfig" type="textarea" :rows="4" placeholder="请输入SQL查询语句或API配置"
            class="code-input full-width">
          </el-input>
        </el-form-item>
        <el-button @click="cereteCode()" type="primary" size="small" class="add-btn"
          v-if="formData.dsType == 'SQL'">解析SQL</el-button>
      </el-form>
    </div>
    ...
    <div class="config-section">
      ...
      <div class="table-container">
        <el-table :data="formData.designTableColumns" :preserve-expanded-content="true" indent row-key="idRow" border
          size="small" class="config-table" @expand-change="onTreeExpandChange" default-expand-all>
          ...
          <el-table-column label="操作" align="center" width="200">
            <template #default="scope">
              <div class="action-buttons" style="gap: 0px">
                ...
                <el-button type="text" size="small" @click="moveDown(scope.$index, scope.row, scope)"
                  class="action-btn">
                  下移
                </el-button>
                ...
                <el-button type="text" size="small" @click="handleDel(scope.row, scope.$index)"
            ...
            <div style="padding: 10px 5px; background: #f9f9f9; min-height: 60px">
              <el-form :model="currentRow" size="small" :inline="true" label-width="auto">
                ...
                <el-form-item label="类型：" style="width: 40%">
                  <el-select v-model="currentRow.fieldFormat" placeholder="请选择" clearable
                ...
                ">
                  <el-select v-model="currentRow.drawerComName" placeholder="请选择" clearable>
                    <el-option v-for="itemOpt in staffTypeOpt" :label="itemOpt.label" :value="itemOpt.value"
                      :key="itemOpt.value" />
                  </el-select>
                </el-form-item>
                ...
                ">
                  <el-select v-model="currentRow.drawerComName" placeholder="请选择" clearable>
                    <el-option v-for="itemOpt in [{ label: '患者360', value: 'patient' }]" :label="itemOpt.label"
                      :value="itemOpt.value" :key="itemOpt.value" />
                  </el-select>
                </el-form-item>
...
const formData = ref({
  ...
  defaultGetTable: 1, //  默认展示表格
  clickFirstRow: true,  // 是否默认选中
...
const formDataFn = (type, bindData) => {
  if (type == "空") {
    return {
      ...
      parentColumn: "",
      childColumn: "",
      defaultGetTable: bindData.defaultGetTable || 1,
      clickFirstRow: true,
    };
  } else {
    return {
      ...
      defaultGetTable: bindData.defaultGetTable || 1,
      clickFirstRow: bindData.clickFirstRow || false,
    };
  }
};
