  ...
  <div class="form-config-panel">
    <el-card class="config-card">
      ...
      <div v-if="formData.fields.length > 0" class="field-list">
        <el-table :data="formData.fields" header-cell-class-name="headerTable" style="width: 100%; height: 100%" border>
          <el-table-column prop="label" label="字段标题" />
          <el-table-column prop="prop" label="字段键名" align="center" />
          <el-table-column prop="component" label="组件" :formatter="(e) => fieldTypesList(e.component)" />
          <el-table-column label="操作" width="300">
            <template #default="{ $index }">
              <div style="display: flex">
                <el-button type="primary" size="small" @click="handSort('up', $index)">上移</el-button>
                <el-button type="primary" size="small" @click="handSort('down', $index)">下移</el-button>
                <el-button type="primary" size="small" @click="openEditFieldDialog($index)">
                  <el-icon>
                    <Edit />
                  </el-icon>
                  编辑
    ...
    <!-- 添加/编辑字段对话框 -->
    <el-dialog v-model="fieldDialogVisible" :title="isEditMode ? '编辑字段' : '添加字段'" width="600px" top="5%"
      destroy-on-close>
      <el-form ref="fieldFormRef" :model="currentField" :rules="fieldRules" label-width="120px" class="field-form">
        <el-form-item label="字段标题：" prop="fieldLabel">
          <el-input v-model="currentField.fieldLabel" placeholder="请输入字段标题" clearable />
        </el-form-item>
        <el-form-item label="字段键名：" prop="fieldCode">
          <el-input v-model="currentField.fieldCode" placeholder="请输入字段名" clearable />
        </el-form-item>
        <el-form-item label="查询方式：" prop="queryType">
          <el-select v-model="currentField.queryType" placeholder="请选择查询方式" clearable>
            <el-option v-for="item in fieldQueryType()" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="组件类型：" prop="fieldType">
          <el-select v-model="currentField.fieldType" placeholder="请选择字段类型" clearable @change="
            currentField.fieldType == 'el-select'
              ? (currentField.source = '自定义')
              : ''
            ">
            <el-option v-for="item in fieldTypesList()" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="
          currentField.fieldType === 'el-select' ||
        ...
        " label="数据来源：">
          <div style="width: 100%; display: flex; justify-content: space-between">
            <el-select v-model="currentField.singleAndMore" placeholder="请选择" style="width: 100px">
              <el-option label="多选" value="0" />
              <el-option label="单选" value="1" />
            </el-select>
            <el-select v-model="currentField.source" placeholder="请选择字段类型" style="width: cacl(100% - 50px)" clearable>
              <el-option v-for="item in optionSource()" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </div>
          <el-input v-model="currentField.sourceValue" v-if="currentField.source != '自定义'" :placeholder="'请输入' + optionSource(currentField.source || '无') + '名称'
            " class="mtForm" clearable />
          <!-- 自定义类型的下拉 -->
          <div style="
              display: flex;
            ...
            <div style="width: 60%">
              <p>选项配置</p>
              <el-select v-model="currentField.customize" placeholder="请选择" style="width: 100%" clearable>
                <el-option v-for="item in customizeList" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </div>
            <div style="margin-left: 10px; width: calc(40% - 10px)">
              <p>基础模板</p>
              <el-select v-model="currentField.base" placeholder="请选择" style="width: 100%" clearable @change="handBase">
                <el-option v-for="item in baseList" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </div>
          </div>
          <!-- 自定义类型的下拉 -->
          <div style="width: 100%; margin-top: 10px" v-if="currentField.source == '自定义'">
            <el-input v-model="newLabel" placeholder="标签" style="width: 120px; margin-right: 10px" />
            <el-input v-model="newValue" placeholder="值" style="width: 120px; margin-right: 10px" />
            <el-button type="primary" @click="addFieldCustomize(newLabel, newValue)">新增</el-button>
            <div v-for="(item, index) in customizeList" :key="index"
              style="margin: 5px 0; display: flex; align-items: center">
              <el-input v-model="item.label" style="width: 120px; margin-right: 10px" />
              <el-input v-model="item.value" style="width: 120px; margin-right: 10px" />
        ...
        ">
          <div style="display: flex; width: 100%; justify-content: space-between">
            <div style="width: 49%">
              键名value:
              <el-input v-model="currentField.optionValue" placeholder="请输入键名value"></el-input>
            </div>
            <div style="width: 49%">
              键名label:<el-input v-model="currentField.optionLabel" placeholder="请输入键名label"></el-input>
            </div>
          </div>
          <div style="width: 100%; margin-top: 10px">
            是否树形结构:
            <el-switch v-model="currentField.moreTree" active-value="1" inactive-value="0"></el-switch>
          </div>
        </el-form-item>
        <el-form-item v-if="currentField.fieldType === 'el-date-picker'" label="时间类型：">
          <el-select v-model="currentField.datePickerType" placeholder="请选择字段类型" clearable>
            <el-option v-for="item in datePickerTypeList()" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="currentField.fieldType === 'el-radio-date'" label="可选值：" prop="defaultType">
          <el-select v-model="currentField.defaultType" placeholder="请选择可选值" clearable multiple>
      ...
      <template #footer>
        <div style="padding: 10px">
          ...
          <el-button type="primary" @click="saveField(fieldFormRef)">确定</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
const attrsFn = (type, item) => {
  ...
  if (fieldTypesList(type) == "时间范围选择") {
    ...
    if (
      ...
      return {
        ...baseAttrs,
        "value-format": item.datePickerType == "year" ? "YYYY" : item.datePickerType == 'month' ? "YYYY-MM" : item.datePickerType == 'date' ? "YYYY-MM-DD" : "YYYY-MM-DD HH:mm:ss",
      };
    }
  } else if (
