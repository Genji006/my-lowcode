<template>
  <div class="panel-tab__content">
    ...
    <!-- 监听器 编辑/创建 部分 -->
    <el-drawer v-model="listenerFormModelVisible" title="执行监听器" :size="`${600}px`" append-to-body destroy-on-close>
      <el-form size="mini" :model="listenerForm" label-width="auto" ref="listenerFormRef" @submit.prevent>
    ...
    <el-dialog title="字段配置" v-model="listenerFieldFormModelVisible" width="600px" append-to-body destroy-on-close>
      <el-form :model="listenerFieldForm" size="mini" label-width="auto" ref="listenerFieldFormRef" style="height: 136px" @submit.prevent>
...
const fieldTypeObject = fieldType
...
const resetListenersList = () => {
  ...
  bpmnElementListeners =
    bpmnElement.businessObject?.extensionElements?.values?.filter(ex => ex.$type === `${prefix}:ExecutionListener`) ?? []
...
const openListenerForm = (listener, index) => {
  // 清空表单
  Object.keys(listenerForm).forEach(key => delete listenerForm[key])
...
const openListenerFieldForm = (field, index) => {
  // 清空表单
  Object.keys(listenerFieldForm).forEach(key => delete listenerFieldForm[key])
...
const saveListenerConfig = async () => {
  ...
  // 保存其他配置
  otherExtensionList = bpmnElement.businessObject?.extensionElements?.values?.filter(ex => ex.$type !== `${prefix}:ExecutionListener`) ?? []
  updateElementExtensions(bpmnElement, otherExtensionList.concat(bpmnElementListeners))
