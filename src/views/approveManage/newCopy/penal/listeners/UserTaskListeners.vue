...
const listenerEventTypeObject = eventType;
...
const resetListenersList = () => {
  ...
  otherExtensionList = [];
  bpmnElementListeners = bpmnElement.businessObject?.extensionElements?.values?.filter(ex => ex.$type === `${prefix}:TaskListener`) ?? [];
...
const saveListenerConfig = async () => {
  ...
  // 保存其他配置
  otherExtensionList = bpmnElement.businessObject?.extensionElements?.values?.filter(ex => ex.$type !== `${prefix}:TaskListener`) ?? [];
  updateElementExtensions(bpmnElement, otherExtensionList.concat(bpmnElementListeners));
...
const saveListenerFiled = async () => {
  ...
  const newField = { ...listenerFieldForm };
  if (editingListenerFieldIndex.value === -1) {
    fieldsListOfListener.value.push(newField);
    listenerForm.fields.push(newField);
  } else {
    fieldsListOfListener.value.splice(editingListenerFieldIndex.value, 1, newField);
    listenerForm.fields.splice(editingListenerFieldIndex.value, 1, newField);
  }
