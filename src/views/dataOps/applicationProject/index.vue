  ...
  <div class="applicationProjectCss">
    ...
    <div class="appProject_content">
      <div style="display: flex;height:50px;align-items:center">
        <szycButton @click="addOrEditDptClick('add','新增项目')"><el-icon class="bmr">
            <Plus />
          </el-icon> 新增 </szycButton>
        <szycButton @click="addOrEditDptClick('edit','编辑项目')" :disabled="rowDataList.length == 0"><el-icon class="bmr">
            <EditPen />
      ...
      <div style="height: calc(100% - 50px);">
        <el-table :data="tableData" ref="tableRef" style="width: 100%;height:100%" header-cell-class-name="headerTable" v-loading="loading" border highlight-current-row @rowClick="handleRowClick">
          ...
          <el-table-column prop="projectName" label="项目名称" width="220" show-overflow-tooltip />
          <el-table-column prop="code" label="项目英文名" width="150" align="center" show-overflow-tooltip />
          <el-table-column label="所属数据源" show-overflow-tooltip>
            <template #default="scope"> {{ sourceIdFn(scope.row.defaultDatasourceId) }} </template>
          </el-table-column>
          <el-table-column prop="state" label="启用否" width="100" align="center">
            ...
            <template #default="scope">
              <el-switch @change="updateStatus($event, scope.row)" v-model="scope.row.state" active-color="#165DFF" inactive-color="#C9CDD4" active-value="1" inactive-value="0" />
            </template>
          </el-table-column>
...
import { listDataSourceApi, listAppProjectApi, addAppProjectApi, updateAppProjectApi, deleteAppProjectApi, enableAppProjectApi } from '@/api/tool.js'
...
const searchItems = ref([
  { label: '项目名称', prop: 'projectName', component: 'el-input', attrs: { placeholder: '请输入名称' }, style: { width: '200px' } },
  { label: '启用否', prop: 'state', component: 'el-select', options: yesOrNoFn(), attrs: { placeholder: '请选择' }, style: { width: '200px' } }
])
...
// 新增编辑表单
const formItems = ref([
  { label: '项目名称：', prop: 'projectName', component: 'el-input', attrs: { placeholder: '请输入项目名称' }},
  { label: '项目英文名：', prop: 'code', component: 'el-input', attrs: { placeholder: '请输入项目英文名' }},
  { label: '所属数据源：', prop: 'defaultDatasourceId', component: 'el-select', attrs: { placeholder: '请选择所属数据源' }, options: [], optionValue: 'id', optionLabel: "connName", },
  { label: '备注：', prop: 'remark', component: 'el-input', attrs: { placeholder: '请输入', type: 'textarea', rows: 6, resize: 'none' } },
  { label: '启用否：', prop: 'state', component: 'el-switch', attrs: { "active-color": "#165DFF", "inactive-color": "#C9CDD4", "active-value": "1", "inactive-value": "0" } },
])
...
const rules = {
  ...
  defaultDatasourceId: [{ required: true, message: '请选择所属数据源', trigger: 'change' }],
}
...
const ListFn = () => {
  ...
  listAppProjectApi(obj).then(res => {
    ...
    if (res.data.length > 0) {
      rowDataList.value = [res.data[0]]
      heightLight(tableRef.value, res?.data[0])
    } else {
      rowDataList.value = []
    }
    loading.value = false
...
const addOrEditDptClick = (type, title) => {
  ...
  if (type == 'add') {
    ...
    formData.defaultDatasourceId = rowDataList.value[0].defaultDatasourceId      //  所属数据源
...
const addDataSource = (type) => {
  if (type == 'add') {
    addAppProjectApi({ ...formData }).then((res) => {
      if (res.code == 200) {
    ...
    updateAppProjectApi({ id: rowDataList.value[0].id, ...formData }).then((res) => {
      if (res.code == 200) {
        ElMessage.success(res.msg)
        ListFn()
        showDialog.value = false
  ...
  if (arr.length > 0) {
    ElMessageBox.confirm(
      ...
      .then(async () => {
        ...
        deleteAppProjectApi(list).then((res) => {
          if (res.code == 200) {
            ElMessage.success(res.msg)
            ListFn()
          }
...
const updateStatus = ($event, row) => {
  ...
  enableAppProjectApi(obj).then((res) => {
    if (res.code == 200) {
      ElMessage.success(res.msg)
      ListFn()
    }
...
onMounted(async () => {
  ...
  let index = formItems.value.findIndex((item) => item.prop == 'defaultDatasourceId')
