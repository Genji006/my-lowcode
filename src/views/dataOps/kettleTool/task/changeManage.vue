...
<template>
  <div class="taskGroupManage-all-class">
    ...
    <div class="taskGroupManage-table-class">
      <tableAndPage ref="tableRef" apiUrl="/task/getTrans" :queryParams="queryParams" :btnList="operationList"
        :designTableColumns="designTableColumns" :showBorder="true" :light="true" @btnClick="buttonClick"
        @selection-change="handleSelectionChange" @rowClick="rowClick" primaryKey="transformationId"
        :reserveSelection="true" v-bind="{ showBorder: 1, pageOpen: 1 }"
        :interfaceType="{ type: 'POST', parameterType: 'special' }">
      </tableAndPage>
    </div>
  </div>
  <!-- 编辑转换 -->
  <SzycDialog :dialogVisible="transNameShow" :close-on-click-modal="false" destroy-on-close title="编辑转换名称" width="25%"
  ...
  <!-- 转换属性 -->
  <SzycDialog :dialogVisible="visibleUserShow" :close-on-click-modal="false" destroy-on-close title="转换属性" width="25%"
...
const rowClick = (val) => {
  ...
  belongToTask.value = convertStringToArray(val.belongToTaskGroup)
  console.log("val高亮", tableRowData.value);
};
...
const handleReset = () => {
  ...
  queryParams.date = [];
  queryParams.name = "";
  // specialHandle(queryParams.date);
  tableRef.value.fetchData("重置");
};
  ...
  switch (val.text) {
    ...
    case "编辑转换":
      router.push({
        ...
        query: {
          ...
          transName: tableRowData.value.name
        }
      });
      break;
    case "编辑名称":
      transNameShow.value = true;
      submitParams.newName = changeName.value;
      break;
    case "转换属性":
    ...
    case "删除":
      ElMessageBox.confirm(
        ...
        try {
          let res = await deleteTrans({ path: selectRowList.value[selectRowList.value.length - 1].directoryName, flag: "transformation" });
...
const setDist = async () => {
  ...
  let params = {
    taskId: tableRowData.value.transformationId,
