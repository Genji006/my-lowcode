<!-- TransGraph.vue -->
<template>
  <div class="trans-graph-container">
...
async function loadComponentTree() {
  try {
    const response = await axios.get('/system/steps.do')
...
async function initGraph() {
  ...
  try {
    ...
    if (typeof mxGraph !== 'undefined') {
      ...
      ElMessage.success('转换加载完成')
    } else {
      ElMessage.warning('mxGraph 库未加载')
    }
  } catch (error) {
    ElMessage.error('加载转换失败')
    console.error(error)
  }
}
