<template>
    <SzycDialog :dialogVisible="dialogVisible" width="45%" @close="dialogVisible = false" title="执行作业"
                    ...
                    <el-tab-pane label="一般" name="baseConfig" style="height: 510px;overflow-y: auto;">
                        <el-form label-position="right" label-width="250px">
                            ...
                            <el-form-item label="Starting point of job">
                                <el-select v-model="form.start_copy_name" placeholder="请选择">
...
import { systemLogLevel, getSlaveSelect, jobEntries } from "@/api/kettleApi/index";
...
const openDialog = async (data, graphXml) => {
...
const getJobs = async (graphXml) => {
    try {
        let res = await jobEntries({ graphXml: graphXml });
        nameList.value = res
    } catch {
