                    ...
                    <el-tabs type="border-card" v-model="activeTab" class="tabItem">
                        <el-form label-position="right" label-width="80px">
                            ...
                            <el-form-item label="目录名">
                                <el-input v-model="form.foldername" placeholder="请输入文件路径">
                                    <template #append>
                            ...
                            <el-form-item>
                                <el-checkbox v-model="form.fail_of_folder_exists" label="如果目录存在则失败" />
                            </el-form-item>
                        </el-form>
                    </el-tabs>
                </el-form>
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const createFolder = stepElements[0];
            // 填充表单字段
            form.stepName = createFolder.getAttribute('label') || '';
            form.foldername = createFolder.getAttribute('foldername') || '';
            form.fail_of_folder_exists = createFolder.getAttribute('fail_of_folder_exists') === 'Y';
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const createFolder = stepElements[0];
            // 更新属性
            createFolder.setAttribute("label", form.stepName);
            createFolder.setAttribute("foldername", form.foldername);
            createFolder.setAttribute("fail_of_folder_exists", form.fail_of_folder_exists ? "Y" : "N");
            // 返回更新后的XML
            return mxUtils.getPrettyXml(createFolder);
        }
