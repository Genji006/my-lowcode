<template>
    <SzycDialog :dialogVisible="dialogVisible" width="35%" @close="dialogVisible = false" title="写入文件"
                ...
                <el-form :model="form" label-width="100px">
                    ...
                    <el-tabs type="border-card" v-model="activeTab" class="tabItem">
                        <el-form label-position="right" label-width="80px">
                            ...
                            <el-form-item>
                                <el-checkbox v-model="form.createParentFolder" label="创建父文件夹" />
                            </el-form-item>

                            <el-form-item>
                                <el-checkbox v-model="form.appendFile" label="追加方式" />
                            </el-form-item>

                            <el-form-item label="编码">
                                <el-select v-model="form.encoding" placeholder="请选择编码" style="width: 100%">
                                    <el-option v-for="charset in charsetOptionList" :key="charset.name"
                                        :label="charset.name" :value="charset.name" />
                                </el-select>
                            </el-form-item>
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const writeToFile = stepElements[0];
            // 填充表单字段
            form.stepName = writeToFile.getAttribute('label') || '';
            form.filename = writeToFile.getAttribute('filename') || '';
            form.createParentFolder = writeToFile.getAttribute('createParentFolder') === 'Y';
            form.appendFile = writeToFile.getAttribute('appendFile') === 'Y';
            form.encoding = writeToFile.getAttribute('encoding') || '';
            form.content = writeToFile.getAttribute('content') || '';
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const writeToFile = stepElements[0];
            // 更新属性
            writeToFile.setAttribute("label", form.stepName);
            writeToFile.setAttribute("filename", form.filename);
            writeToFile.setAttribute("createParentFolder", form.createParentFolder ? "Y" : "N");
            writeToFile.setAttribute("appendFile", form.appendFile ? "Y" : "N");
            writeToFile.setAttribute("encoding", form.encoding);
            writeToFile.setAttribute("content", form.content);
            // 返回更新后的XML
            return mxUtils.getPrettyXml(writeToFile);
        }
