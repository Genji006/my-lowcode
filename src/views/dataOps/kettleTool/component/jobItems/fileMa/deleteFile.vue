<template>
    <SzycDialog :dialogVisible="dialogVisible" width="35%" @close="dialogVisible = false" title="删除一个文件"
                ...
                <el-form :model="form" label-width="100px">
                    ...
                    <el-tabs type="border-card" v-model="activeTab" class="tabItem">
                        <el-form label-position="right" label-width="80px">
                            ...
                            <el-form-item>
                                <el-checkbox v-model="form.fail_if_file_exists" label="如果文件不存在则失败" />
                            </el-form-item>
                        </el-form>
                    </el-tabs>
                </el-form>
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const deleteFile = stepElements[0];
            // 填充表单字段
            form.stepName = deleteFile.getAttribute('label') || '';
            form.filename = deleteFile.getAttribute('filename') || '';
            form.fail_if_file_exists = deleteFile.getAttribute('fail_if_file_exists') === 'Y';
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const deleteFile = stepElements[0];
            // 更新属性
            deleteFile.setAttribute("label", form.stepName);
            deleteFile.setAttribute("filename", form.filename);
            deleteFile.setAttribute("fail_if_file_exists", form.fail_if_file_exists ? "Y" : "N");
            // 返回更新后的XML
            return mxUtils.getPrettyXml(deleteFile);
        }
