<template>
    <SzycDialog :dialogVisible="dialogVisible" width="35%" @close="dialogVisible = false" title="检查一个文件是否存在"
            ...
            <div class="mxgraphForm">
                <el-form :model="form" label-width="100px">
                    ...
                    <el-tabs type="border-card" v-model="activeTab" class="tabItem">
                        <el-form label-position="right" label-width="100px" style="margin: 20px 0;">
    ...
    <FileExplorerWindow v-model:visible="showFileExplorer" :extension="24" @ok="handleFileSelected" />
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const fileExists = stepElements[0];
            // 填充表单字段
            form.stepName = fileExists.getAttribute('label') || '';
            form.filename = fileExists.getAttribute('filename') || '';
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const fileExists = stepElements[0];
            // 更新属性
            fileExists.setAttribute("label", form.stepName);
            fileExists.setAttribute("filename", form.filename);
            // 返回更新后的XML
            return mxUtils.getPrettyXml(fileExists);
        }
