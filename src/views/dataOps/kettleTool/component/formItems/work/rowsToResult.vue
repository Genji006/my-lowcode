<template>
    <SzycDialog :dialogVisible="dialogVisible" width="30%" @close="dialogVisible = false" title="复制记录到结果"
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const rowsToResult = stepElements[0];
            console.log("rowsToResult", rowsToResult);
            // 填充表单字段
            form.stepName = rowsToResult.getAttribute('label') || '';
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const rowsToResult = stepElements[0];
            // 更新属性
            rowsToResult.setAttribute("label", form.stepName);
            return mxUtils.getPrettyXml(rowsToResult);
        }
