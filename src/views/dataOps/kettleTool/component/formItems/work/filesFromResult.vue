<template>
    <SzycDialog :dialogVisible="dialogVisible" width="30%" @close="dialogVisible = false" title="从结果获取文件"
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const filesFromResult = stepElements[0];
            console.log("filesFromResult", filesFromResult);
            // 填充表单字段
            form.stepName = filesFromResult.getAttribute('label') || '';
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const filesFromResult = stepElements[0];
            // 更新属性
            filesFromResult.setAttribute("label", form.stepName);
            return mxUtils.getPrettyXml(filesFromResult);
        }
