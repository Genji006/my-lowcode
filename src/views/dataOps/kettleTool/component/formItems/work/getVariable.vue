...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
  try {
    ...
    if (stepElements.length > 0) {
      const getVariable = stepElements[0];
      console.log("getVariable", getVariable);
      // 填充表单字段
      form.stepName = getVariable.getAttribute("label") || "";
      form.fields = JSON.parse(getVariable.getAttribute("fields") || "[]");
    }
  } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
  try {
    ...
    if (stepElements.length > 0) {
      const getVariable = stepElements[0];
      // 更新属性
      getVariable.setAttribute("label", form.stepName);
      getVariable.setAttribute("fields", JSON.stringify(form.fields));
      // 返回更新后的XML
      return mxUtils.getPrettyXml(getVariable);
    }
