<template>
    <SzycDialog :dialogVisible="dialogVisible" width="30%" @close="dialogVisible = false" title="行样本"
            ...
            <div class="mxgraphForm">
                <el-form :model="form" label-width="100px">
                    ...
                    <el-form-item label="行范围：">
                        <el-input v-model="form.linesrange" placeholder="请输入"></el-input>
                    </el-form-item>
                    <el-form-item label="行号字段名：">
                        <el-input v-model="form.linenumfield" placeholder="请输入"></el-input>
                    </el-form-item>
                </el-form>
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const sampleRows = stepElements[0];
            console.log("sampleRows", sampleRows);
            // 填充表单字段
            form.stepName = sampleRows.getAttribute('label') || '';
            form.linesrange = sampleRows.getAttribute('linesrange') || '';
            form.linenumfield = sampleRows.getAttribute('linenumfield') || '';
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const sampleRows = stepElements[0];
            // 更新属性
            sampleRows.setAttribute("label", form.stepName);
            sampleRows.setAttribute("linesrange", form.linesrange);
            sampleRows.setAttribute("linenumfield", form.linenumfield);

            return mxUtils.getPrettyXml(sampleRows);
        }
