<template>
    <SzycDialog :dialogVisible="dialogVisible" width="30%" @close="dialogVisible = false" title="空操作 (什么也不做)"
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const dummyElement = stepElements[0];
            console.log("dummyElement", dummyElement);
            // 填充表单字段
            form.stepName = dummyElement.getAttribute('label') || '';
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const dummyElement = stepElements[0];
            // 更新属性
            dummyElement.setAttribute("label", form.stepName);
            return mxUtils.getPrettyXml(dummyElement);
        }
