<template>
    <SzycDialog :dialogVisible="dialogVisible" width="30%" @close="dialogVisible = false" title="设置字段值"
                ...
                <el-form :model="form" label-width="100px">
                    ...
                    <el-tabs type="border-card" v-model="activeTab" class="tabItem">
                        <RightClickTable ref="fileTableRef" v-model="form.fields" :columns="filedColumns"
                            :showIndex="true" :height="200" style="width: 100%" button-text="获取字段"
                            :needSpecialFunction="true" @parent-function="fieldParentFunction">
                        </RightClickTable>
                    </el-tabs>
                </el-form>
...
import { inputOutputFields, valueMeta, previousSteps } from "@/api/kettleApi/index";
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            ...
            form.fields = JSON.parse(value.getAttribute('fields') || '[]');
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            ...
            value.setAttribute("fields", JSON.stringify(form.fields));
            // 返回更新后的XML
    ...
    switch (action) {
        case 'addNew':
            ...
            formattedData.forEach(item => {
                if (!form.fields.some(existingItem => existingItem.keyStream1 === item.keyStream1)) {
...
const filedColumns = computed(() => [
    ...
    {
        prop: "replaceby",
        label: "Replace by value from filed",
        type: "select",
        options: partitioningFieldList.value,
        valueKey: "name",
        labelKey: "name",
