<template>
    <SzycDialog :dialogVisible="dialogVisible" width="40%" @close="dialogVisible = false" title="根据Java代码过滤记录"
                ...
                <el-form :model="form" label-width="100px">
                    ...
                    <el-tabs type="border-card" v-model="activeTab" class="tabItem">
                        <el-form label-position="right" label-width="230px">
                            <el-form-item label="接收匹配的行的步骤（可选）：">
                            ...
                            <el-form-item label="接收不匹配的行的步骤（可选）：">
                                <el-select v-model="form.send_false_to" style="width: 100%;" placeholder="">
                                    <el-option v-for="item in targetStepList" :key="item.name" :label="item.name"
                                        :value="item.name"></el-option>
                                </el-select>
                            </el-form-item>
                            <el-form-item label="条件（Java表达式）：">
                                <el-input v-model="form.condition" placeholder="请输入Java表达式" type="textarea" rows="8"></el-input>
                            </el-form-item>
                        </el-form>
                    </el-tabs>
                </el-form>
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            ...
            form.send_true_to = javaFilterElement.getAttribute('send_true_to') || '';
            form.send_false_to = javaFilterElement.getAttribute('send_false_to') || '';
            form.condition = decodeURIComponent(javaFilterElement.getAttribute('condition') || '');
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            ...
            javaFilterElement.setAttribute("send_true_to", form.send_true_to);
            javaFilterElement.setAttribute("send_false_to", form.send_false_to);
            javaFilterElement.setAttribute("condition", encodeURIComponent(form.condition));
            // 返回更新后的XML
