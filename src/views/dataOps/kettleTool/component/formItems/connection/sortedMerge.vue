...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
  try {
    ...
    if (stepElements.length > 0) {
      const sortedMerge = stepElements[0];
      console.log("sortedMerge", sortedMerge);
      // 填充表单字段
      form.stepName = sortedMerge.getAttribute("label") || "";
      form.fields = JSON.parse(sortedMerge.getAttribute("fields") || "[]");
    }
  } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
  try {
    ...
    if (stepElements.length > 0) {
      const sortedMerge = stepElements[0];
      // 更新属性
      sortedMerge.setAttribute("label", form.stepName);
      sortedMerge.setAttribute("fields", JSON.stringify(form.fields));
      // 返回更新后的XML
      return mxUtils.getPrettyXml(sortedMerge);
    }
