<template>
    <SzycDialog :dialogVisible="dialogVisible" width="45%" @close="dialogVisible = false" title="更新"
            ...
            <div class="mxgraphForm">
                <el-form :model="form" label-width="100px">
                    ...
                    <el-tabs type="border-card" v-model="activeTab" class="tabItem">
                        <el-tab-pane label="一般" name="baseConfig" style="height: 500px;overflow-y: auto;">
                            <el-form label-position="right" label-width="200px">
                                ...
                                <el-form-item label="忽略查询失败：">
                                    <el-checkbox v-model="form.skip_lookup"></el-checkbox>
                                </el-form-item>
                                <el-form-item label="跳过查询：">
                                    <el-checkbox v-model="form.error_ignored"></el-checkbox>
                                </el-form-item>
                                <el-form-item label="标志字段（key found）：">
                                    <el-input v-model="form.ignore_flag_field" placeholder=""></el-input>
                                </el-form-item>
                            </el-form>
                        </el-tab-pane>
                        <el-tab-pane label="查询字段" name="searchConfig" style="height: 500px;overflow-y: auto;">
                        ...
                        <el-tab-pane label="更新字段" name="updateConfig" style="height: 500px;overflow-y: auto;">
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const update = stepElements[0];
            console.log("update", update);
            // 填充表单字段
            form.stepName = update.getAttribute('label') || '';
            form.connection = update.getAttribute('connection') || '';
            form.schema = update.getAttribute('schema') || '';
            form.table = update.getAttribute('table') || '';
            form.commit = update.getAttribute('commit') || '';
            form.use_batch = update.getAttribute('use_batch') === 'Y';
            form.tablename_in_field = update.getAttribute('tablename_in_field') === 'Y';
            form.tablename_field = update.getAttribute('tablename_field') || '';
            form.skip_lookup = update.getAttribute('skip_lookup') === 'Y';
            form.error_ignored = update.getAttribute('error_ignored') === 'Y';
            form.ignore_flag_field = update.getAttribute('ignore_flag_field') || '';
            searchFields.value = JSON.parse(update.getAttribute('searchFields') || '[]');
            updateFields.value = JSON.parse(update.getAttribute('updateFields') || '[]');
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const update = stepElements[0];
            // 更新属性
            update.setAttribute("label", form.stepName);
            update.setAttribute("connection", form.connection);
            update.setAttribute("schema", form.schema);
            update.setAttribute("table", form.table);
            update.setAttribute("commit", form.commit);
            update.setAttribute("use_batch", form.use_batch ? 'Y' : 'N');
            update.setAttribute("tablename_in_field", form.tablename_in_field ? 'Y' : 'N');
            update.setAttribute("tablename_field", form.tablename_field);
            update.setAttribute("skip_lookup", form.skip_lookup ? 'Y' : 'N');
            update.setAttribute("error_ignored", form.error_ignored ? 'Y' : 'N');
            update.setAttribute("ignore_flag_field", form.ignore_flag_field);
            update.setAttribute("searchFields", JSON.stringify(searchFields.value));
            update.setAttribute("updateFields", JSON.stringify(updateFields.value));
            // 返回更新后的XML
            return mxUtils.getPrettyXml(update);
        }
