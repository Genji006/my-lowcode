<template>
    <SzycDialog :dialogVisible="dialogVisible" width="50%" @close="dialogVisible = false" title="增加常量"
                ...
                <el-form :model="form" label-width="100px">
                    ...
                    <el-tabs type="border-card" v-model="activeTab" class="tabItem">
                        <RightClickTable v-model="form.fields" :columns="fieldDataColumns" :showIndex="true"
                            style="width: 100%" :needSpecialFunction="false" @parent-function="fieldParentFunction()">
                        </RightClickTable>
                    </el-tabs>
                </el-form>
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const constant = stepElements[0];
            console.log("constant", constant);
            // 填充表单字段
            form.stepName = constant.getAttribute('label') || '';
            form.fields = JSON.parse(constant.getAttribute('fields') || '[]');
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const constant = stepElements[0];
            // 更新属性
            constant.setAttribute("label", form.stepName);
            constant.setAttribute("fields", JSON.stringify(form.fields));
            // 返回更新后的XML
            return mxUtils.getPrettyXml(constant);
        }
...
const fieldDataColumns = computed(() => [
    ...
    { prop: "precision", label: "精确" },
    { prop: "currency", label: "当前的" },
    { prop: "decimal", label: "10进制的" },
    { prop: "group", label: "组" },
    { prop: "nullif", label: "值" },
    { prop: "set_empty_string", label: "设为空串？", type: "select", options: [{ label: "是", value: "Y" }, { label: "否", value: "N" }] },
]);
