      ...
      <div class="mxgraphForm">
        <el-form :model="form" label-width="100px">
          ...
          <el-form-item label="key字段：">
            <el-input v-model="form.typefield" placeholder="请输入"></el-input>
          </el-form-item>
          <!-- 其他表单项 -->
        </el-form>
        <div>
...
const fieldDataColumns = computed(() => [
  ...
  { prop: "value", label: "key 值", width: 200 },
  { prop: "norm", label: "value 字段", width: 200 },
]);
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
  try {
    ...
    if (stepElements.length > 0) {
      ...
      form.typefield = rowtoColumn.getAttribute("typefield") || "";
...
const updateStepInfoXml = (stepInfoXml) => {
  try {
    ...
    if (stepElements.length > 0) {
      ...
      rowtoColumn.setAttribute("typefield", form.typefield);
