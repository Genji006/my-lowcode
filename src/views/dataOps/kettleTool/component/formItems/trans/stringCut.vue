<template>
    <SzycDialog :dialogVisible="dialogVisible" width="50%" @close="dialogVisible = false" title="剪切字符串"
...
const parseStepInfoAndPopulateForm = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const stringCut = stepElements[0];
            console.log("stringCut", stringCut);
            // 填充表单字段
            form.stepName = stringCut.getAttribute('label') || '';
            form.fields = JSON.parse(stringCut.getAttribute('fields') || '[]');
        }
    } catch (error) {
...
const updateStepInfoXml = (stepInfoXml) => {
    try {
        ...
        if (stepElements.length > 0) {
            const stringCut = stepElements[0];
            // 更新属性
            stringCut.setAttribute("label", form.stepName);
            stringCut.setAttribute("fields", JSON.stringify(form.fields));
            // 返回更新后的XML
            return mxUtils.getPrettyXml(stringCut);
        }
...
const fieldDataColumns = computed(() => [
    ...
    { prop: "out_stream_name", label: "输出流字段" },
    { prop: "cut_from", label: "起始位置" },
    { prop: "cut_to", label: "结束位置" },
]);
    ...
    switch (action) {
        case 'addNew':
            ...
            formattedData.forEach(item => {
                if (!form.fields.some(existingItem => existingItem.in_stream_name === item.in_stream_name)) {
