<template>
  <div style="display: flex; width: 100%" v-loading="treeLoading">
    <el-tree-v2 show-checkbox ref="leftTreeDataRef" :check-strictly="true" v-show="lineExper" class="treeCss" v-loading="loadingLeft" :height="heightTree" value-key="uniqueCode" :data="leftTreeData" @check-change="(data, checked) => handleCheck(data, checked, 'left')" :props="setObj">
    ...
    <el-tree-v2 show-checkbox ref="rightTreeDataRef" class="treeCss" :check-strictly="true" v-loading="loadingRight" @check-change="(data, checked) => handleCheck(data, checked, 'right')" :height="heightTree" value-key="uniqueCode" :data="rightTreeData" :props="setObj">
      <template #default="{ node, data }">
        <el-icon class="node-icon" :class="{ 'is-leaf': node.isLeaf }">
          <Document v-if="node.isLeaf" />
          <Folder v-else-if="!node.expanded" />
...
import { codeGroupTreeApi } from "@/api/tool";
...
function addBindToSelected(data, selectedIds, value) {
  ...
  return data.map((item) => {
    ...
    if (newItem.children && newItem.children.length > 0) {
      newItem.children = addBindToSelected(newItem.children, selectedIds, value);
    }
    return newItem;
  });
}
