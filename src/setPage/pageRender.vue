...
const resolveLinkage = (config) => {
    ...
    dependencies.forEach(depId => {
        const sourceData = globalDataStore.value[depId]; // 获取上游组件的全量数据
        ...
        if (mappings && mappings.length > 0) {
            ...
            if (sourceData.value !== undefined) {
                combinedParams[depId] = sourceData.value; // 简单值
...
const getPosStyle = (comp) => {
    return {
        ...
        // 高度：如果是数字加px，如果是auto或百分比直接用
        height: typeof comp.style.height === 'number' ? comp.style.height + 'px' : (comp.style.height || 'auto'),
