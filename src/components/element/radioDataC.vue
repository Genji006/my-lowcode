<template>
  <div style="display: flex; align-items: center" class="radio-group-container">
    <el-radio-group v-model="dataDefaultType" style="margin-right: 10px;flex-wrap: nowrap" class="radio-group-style">
      <el-radio
        label="year"
        v-if="defaultType?.includes('year')"
        :disabled="!defaultType?.includes('year') && defaultType?.length"
        >年</el-radio
      >
      <el-radio  v-if="defaultType?.includes('quarter')" label="quarter" :disabled="!defaultType?.includes('quarter')"
        >季</el-radio
      >
      <el-radio label="months" v-if="defaultType?.includes('months')" :disabled="!defaultType?.includes('months')"
        >月</el-radio
      >
      <el-radio label="date" v-if="defaultType?.includes('date')" :disabled="!defaultType?.includes('date')"
        >日</el-radio
      >
    </el-radio-group>
    <el-date-picker
    ...
    <el-date-picker
      ...
      :align-center="true"
      class="date-range-pickerCopy"
      style="display: flex; align-items: center; margin-left: 10px"
      v-if="dataDefaultType == 'date'"
      value-format="YYYY-MM-DD"
    ></el-date-picker>
    <div class="el-quarter-picker" v-if="dataDefaultType == 'quarter'">
      <el-popover :visible="visible" :width="200" trigger="click">
        <div>
          <div class="top-box">
...
const dateValueChange = (val) => {
  ...
  } else if (dataDefaultType.value === "date") {
    if (!val || val.length == 0) {
      // 避免循环更新
...
watch(
  ...
  (val) => {
    ...
    if (val == "year") {
      ...
      if (!Array.isArray(dateValue.value) || dateValue.value.length !== 0) {
        dateValue.value = [];
      }
    }
  },
...
watch(
  ...
  (val) => {
    ...
    } else if (props.defaultType?.includes("date")) {
      dataDefaultType.value = "date";
    } else {
