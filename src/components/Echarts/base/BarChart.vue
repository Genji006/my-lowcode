...
// 获取数据
const fetchData = () => {
  let arr = Array.from({ length: 10 }, (v, k) =>
    Math.floor(Math.random() * 100)
  );
  let arrs = Array.from({ length: 10 }, (v, k) =>
    Math.floor(Math.random() * 100)
  );
  let series = [
    { name: "谷歌", data: arr },
...
//  处理series
const seriesFn = (arr, colors, list = [], direction, stack) => {
  let lens = arr.length;
...
const chartOption = computed(() => {
  // console.log(props.showLegend, "props.showLegend")
  let series = seriesFn(
